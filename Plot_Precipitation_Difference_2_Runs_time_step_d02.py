# Michael Wasserstein
# Plot_Precipitation_Difference_2_runs_time_step_d02.py
# 11/22/2024
...
# python /uufs/chpc.utah.edu/common/home/u1371671/WRF/Plotting_Scripts/Plot_Precipitation_Difference_2_runs_time_step_d02.py -r1 20 -r2 19 -p 12
...
Fig_dir1 = '/uufs/chpc.utah.edu/common/home/u1371671/WRF/Figures_{}/wrf_{}/Precipitation_Difference_d02_{}_{}_time_step/'.format(path, run_number1, run_number1, run_number2)
...
# paths for saving fig
Fig_dir2 = '/uufs/chpc.utah.edu/common/home/u1371671/WRF/Figures_{}/wrf_{}/Precipitation_Difference_d02_{}_{}_time_step/'.format(path, run_number2, run_number1, run_number2)
...
# Loop through all time steps (but don't include very last cause you cant do an i+1 for that one)
for i in range(RAINNC_d02_run2.shape[0]-1):  
    
    # Times for title
    start_time = datetime.datetime.strftime(pd.to_datetime(RAINNC_d02_run2.Time[i].values), '%b %-d, %Y %H:%M UTC')
    end_time = datetime.datetime.strftime(pd.to_datetime(RAINNC_d02_run2.Time[i+1].values), '%b %-d, %Y %H:%M UTC')
    
    # Times for saving
    start_time_save = datetime.datetime.strftime(pd.to_datetime(RAINNC_d02_run2.Time[i].values), '%Y%m%d%H%M')
    end_time_save = datetime.datetime.strftime(pd.to_datetime(RAINNC_d02_run2.Time[i+1].values), '%Y%m%d%H%M')
    
    # Determine amount of precip int the hour
    RAINC_d02_run2_timestep_precip = RAINNC_d02_run2[i+1].values - RAINNC_d02_run2[i].values
    RAINC_d02_run1_timestep_precip = RAINNC_d02_run1[i+1].values - RAINNC_d02_run1[i].values
    
    # Calculate the precipitation the difference for the 1 hr period
    timestep_precipitation_difference = RAINC_d02_run2_timestep_precip - RAINC_d02_run1_timestep_precip
    ...
    ################################ ax1 - run1 d02-d03 ############################################
    ################################################################################################
    # Geopolitical boundaries and coasts
    ...
    # Plot PRecipitation
    plot = ax1.contourf(RAINNC_d02_run1.XLONG.values, RAINNC_d02_run1.XLAT.values, RAINC_d02_run1_timestep_precip, cmap = cmap, 
    ...
    # Plot PRecipitation
    plot = ax2.contourf(RAINNC_d02_run2.XLONG.values, RAINNC_d02_run2.XLAT.values, RAINC_d02_run2_timestep_precip, cmap = cmap, 
                         levels = levels, extend = 'max',zorder = 20,transform=ccrs.PlateCarree(),alpha = 1)
    ...
    plot = ax3.contourf(RAINNC_d02_run2.XLONG.values, RAINNC_d02_run2.XLAT.values, timestep_precipitation_difference, cmap = 'bwr', 
    ...
    plt.suptitle(f'Hourly Precipitation {start_time}\u2013{end_time}', y = 0.70)
