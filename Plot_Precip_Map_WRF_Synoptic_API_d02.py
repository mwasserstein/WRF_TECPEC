...
# Plot settings
levels = np.arange(0,30.1,0.5) # Specify levels - this could take some work
...
for i in range(len(stn_precip_accums)):
    text = ax.text(stn_lons[i], stn_lats[i], s = round(stn_precip_accums[i], 1), size = 8,
                   ...
                   ec='k',
                   fc=cmap(stn_precip_accums[i] / levels.max()), # This code gets the color in the colormap corresponding with the precipitation accumulation
